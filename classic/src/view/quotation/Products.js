Ext.define('KJERP.view.quotation.Products', {
	extend: 'Ext.tree.Panel',
	requires:[
		'Ext.toolbar.Spacer',
		'KJERP.model.QuotationItem',
		...
		'KJERP.view.widget.ComponentEditForm',
        'Ext.data.*',
        'Ext.tree.*'		
	],
	...
	initComponent:function() {
		...
		me.store = new Ext.data.TreeStore({
            model:'KJERP.model.QuotationItem',
            remoteSort: false,
            autoLoad:false,
            autoDestroy:false,
		...
		me.store.on('datachanged', function(store) {
			...
			if (Ext.ComponentQuery.query('quotationform')) {
				var countButton = me.up('quotationform').down('button[action=count]');
				countButton.setText(countLabel.text);
			}
			me.updatePrice();
		});
		...
		/*
		me.on('itemclick', function( view, record, item, index, event, eOpts ) {
			if (event.getTarget('a.editComponent')) {
				event.stopEvent();
				_FUNCTION.showEditComponent(record);
			}
		});
		*/
	}
